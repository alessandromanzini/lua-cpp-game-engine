...
bool GameEngine::Run( HINSTANCE hInstance, int cmdShow, const wchar_t* scriptPath )
{
	// set the instance member variable of the game engine
	SetInstance( hInstance );

	// Game initialization
	m_GamePtr->Initialize( scriptPath );
...
void GameEngine::MessageBoxCSTR( const std::string& message ) const
{
	MessageBox( tstring( message.begin(), message.end() ) );
}
...
POINT GameEngine::AngleToPoint( int left, int top, int right, int bottom, int angle ) const
{
	...
	{
		pt.x = left;					pt.y = top + ( height / 2 );
	}
	else if ( angle == 270 )
	{
		pt.x = left + ( width / 2 );	pt.y = top + height;
	}
...
void Audio::PlayFromTo( int msecStart, int msecStop )
{
	if ( !m_Playing )
	{
		m_Playing = true;
...
void Button::SetFont( Font* pFont )
{
	m_Font = pFont->GetHandle( );
}
