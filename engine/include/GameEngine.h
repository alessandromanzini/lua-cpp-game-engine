...
#include "DataTypes.h"					// common data types
...
public:
	...
	bool Run( HINSTANCE hInstance, int cmdShow, const wchar_t* scriptPath );
	...
	void MessageBox( MyOtherType value ) const
	{
		MessageBox( to_tstring( value ) );
	}
 	void MessageBoxCSTR( const std::string& message ) const;
	...
	bool FillWindowRect( COLORRGB color )const
	{
		return FillWindowRect( static_cast<COLORREF>( color ) );
	}
	...
	{
		return FillRect( rect.left, rect.top, rect.right, rect.bottom );
	}
	bool DrawRoundRect( const RECT& rect, int radius )										const
	{
		return DrawRoundRect( rect.left, rect.top, rect.right, rect.bottom, radius );
	}
	bool FillRoundRect( const RECT& rect, int radius )										const
	{
		return FillRoundRect( rect.left, rect.top, rect.right, rect.bottom, radius );
	}
...
public:
	...
	{
		SetBackcolor( static_cast<COLORREF>( color ) );
	}
	void		SetForecolor( COLORREF color );
	void		SetForecolor( COLORRGB color )
	{
		SetForecolor( static_cast<COLORREF>( color ) );
	}
...
public:
	...
	void PlayFromTo( int msecStart, int msecStop );
	void Pause( );
	void Stop( );
...
public:
	...
	void SetTransparencyColor( COLORRGB color )
	{
		SetTransparencyColor( static_cast<COLORREF>( color ) );
	}
	void SetOpacity( int );
	...
	COLORRGB GetTransparencyColorStruct( ) const
	{
		return static_cast<COLORRGB>( GetTransparencyColor( ) );
	}
...
public:
	...
	HitRegion( const Bitmap* bmpPtr, COLORREF cTransparent, COLORREF cTolerance ); // Use this create to create a hitregion from a bitmap
	HitRegion( const Bitmap* bmpPtr )
		: HitRegion( bmpPtr, RGB( 255, 0, 255 ), 0 ) { }
